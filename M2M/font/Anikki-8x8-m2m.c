/* Native 8x8 ROM generator from the Anikki-16x16 source font,
   ...
   Generate the native 8x8 .rom file used by the M2M OSM renderer.
...
int main(void)
{
    ...
    for (glyph = 0; glyph < GLYPH_COUNT; glyph++)
    {
        ...
        {
            for (x = 0; x < NATIVE_DX; x++)
                bin[x] = source_pixel(glyph, 2u * x, 2u * y) ? '1' : '0';
    ...
    printf("Generated validated native 8x8 ROM: %u glyphs, %u rows, %u bits/row\n",
           GLYPH_COUNT, GLYPH_COUNT * NATIVE_DY, NATIVE_DX);
    return EXIT_SUCCESS;
}
