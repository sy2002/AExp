...
| WIP-V1-A9     | 07/13/26 | tbd     | True analog picture position: pan_x/pan_y move the complete analog picture (menu included) in Standard and both 15 kHz modes by shifting the sync phase (issue #5). The former VGA edge offsets are now honestly named analog overscan (they crop/reveal border, they never panned) and got hardware safety clamps. New v4 aexp_screen.cfg (v3 still accepted, pan = 0) and a friendlier aexp_screen_cfg.py: grouped editing, direction hints, --pan-x/--pan-y, --reset, --copy-from, automated tests.
