...
- **Triple buffering stays off**: the firmware's HDMI Filter dispatcher writes only ascal modes 0x0000-0x0004 (`M2M/rom/sysdef.asm:119-123`, `m2m-rom.asm` HDMI_FLT_TABLE) — bit 3 (triple buffering) is never set. Weave works in the single buffer. **HDMI: Flicker-free (§3.4) is a second, independent reason it must stay off**: the flicker-free loop keys on the read pointer returning to base exactly once per output frame, which only holds with a single framebuffer. Do not enable triple buffering without re-checking BOTH the HyperRAM map (3 × 2 MB would collide with the ADF window at 4 MB) and the flicker-free measurement.
...
- **The VGA "Standard" mode stays field-blind.** The M2M scandoubler (`video_mixer.sv`/`scandoubler.v`) has no field input and line-doubles each field independently — laced content bobs on the scandoubled 31 kHz output. The remedy is the OSM `VGA:` menu (three-way radio, decoded in `mega65.vhd` like C64MEGA65): **Standard** (scandoubler on, default), **15 kHz with HS/VS** and **15 kHz with CSYNC** — the two retro modes bypass the scandoubler entirely, and since minimig's half-line vsync offset passes through untouched, a 15 kHz CRT weaves interlace natively (no flicker fixer needed, the most authentic picture). In the retro modes `main.vhd` halves the OSM overlay sampling CE to 14.19 MHz (`video_retro15khz_i`). MiSTer's automatic variant (`Minimig.sv:678`: `scandoubler = ... & ~lace`, ideal for CRTs but fatal for VGA LCDs mid-demo) was deliberately not adopted; minimig's `lace` output remains unconnected in `main.vhd` should an "auto" option ever be wanted. Note: on a VGA LCD the 15 kHz modes show no picture at all, including the OSM — recovery is via HDMI, which always stays alive.
...
- **Frame-rate seam — fixed by "HDMI: Flicker-free" (issue #12).** The core's native 49.92 Hz vs HDMI's exact 50.000 Hz makes the ascal single-buffer read/write pointers drift and cross over, dropping/repeating one frame roughly every 12 s (a judder/tear on scrollers; on laced content a one-frame comb blip). The **HDMI: Flicker-free** toggle (top-level OSM item, default ON) removes it: `clk.vhd` adds a second MMCM at 28.4375 MHz (a hair *above* 50 Hz) and a glitch-free `BUFGMUX_CTRL` dithers the core clock between native (below 50) and fast (above 50) under the ascal over/underflow feedback, so the core's *time-average* frame rate becomes exactly 50.000 Hz and the pointers never cross. Auto-toggling interlace already averages exactly 50.000 Hz, so the loop simply settles on native there (zero ongoing dither). Direction note vs C64MEGA65: the C64's native is *above* 50 so its twin is slower; the Amiga's native is *below* 50 so the twin is *faster*.
  - **Turn it OFF for analog VGA / retro 15 kHz.** `video_clk_o` is the same dithered clock, so each glitch-free switch steps the analog H-sync *frequency* (not a glitch, but an analog H-PLL still has to chase it → the C64-documented "underwater"/sporadic sync loss). A 15 kHz CRT also weaves interlace natively and wants authentic pitch. Guidance: FF ON for HDMI, FF OFF for the analog outputs (also stated in About & Help).
  - **Emulated timekeeping runs +5.8 s/hour while locked.** The locked mean core clock is +0.16 % (an inaudible +2.77-cent pitch shift), so CIA-TOD / vblank-counted software clocks gain ~5.8 s/hour — same class as C64MEGA65. Interacts with the battery RTC (issue #13): a `SetClock`-loaded time drifts while FF is ON.
  - **Uncorrectable set (this 2-clock increment):** content whose time-averaged rate is *above* 50.000 Hz — sustained 312-line progressive (`VPOSW` LOF held) or parity-forced-short lace (a laced demo pegging `long_frame`) — cannot be pulled *down* by the {native, fast} pair and keeps the old ~12 s seam (no regression vs before). A future Increment 2 would add a third "slow" MMCM (28.3125 MHz) to also correct these; rare and self-inflicted in practice.
...
- **Behavior checklist for hardware verification**: laced static screens stable at full vertical resolution; progressive demos/games pixel- identical to the previous build (INTER is neutral while `i_fl` is constant); laced↔progressive switches recover within a few frames; OSM overlay unaffected; VGA "Standard" output unchanged (still bobs on laced — known limitation, see §3.4).
- **VGA 15 kHz modes** (needs a 15 kHz-capable display or SCART CRT): switch `VGA:` to "15 kHz with CSYNC" (SCART) or "15 kHz with HS/VS"; verify picture and readable OSM on the CRT, native interlace on laced content (stable 512-line picture with the CRT's own field weave), and that a plain VGA LCD in these modes shows nothing while HDMI keeps working (recovery path).
